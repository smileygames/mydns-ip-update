...
```
bash <( curl -fsSL https://github.com/smileygames/mydns-ip-update/releases/download/v1.12/mydns-ip-install.sh )
```
▼最初に初期設定を行ってください。
```
...
▼アンインストールコマンド
```
bash <( curl -fsSL https://github.com/smileygames/mydns-ip-update/releases/download/v1.12/mydns-ip-uninstall.sh )
```
